...
3. 每个执行 agent 的提示词必须包含：任务文件路径（`plans/TASK-<slug>.md`）、目标、涉及文件、验收标准、上游任务的产出
4. 小任务（单文件 <30 行的改动）不派 agent，内联做掉，省编排开销
5. 执行 agent 命中 WORKFLOW.md 的硬停止条件时：收拢它的状态报告，升级给用户判断，不擅自重试
