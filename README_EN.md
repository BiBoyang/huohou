...
| `huohou-collab-mode` | Collab mode entry: WORKFLOW.md three-role protocol (Owner/Planner/Executor) + file-based prompt dispatch (Planner emits dispatch batches, Owner dispatches to standalone sessions) + evidence-based acceptance | `协作模式` / `退出协作模式` |
